...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnFirstTwoHiredEmployees()
    {
        ...
        var firstTwo = emps.OrderByDescending(e => e.HireDate).Take(2).ToList(); 
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, eouter => eouter.Mgr, einner => einner.EmpNo,
            (employee, manager) => new{Employee = employee.EName, Manager = manager.EName});
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { Emp = e, Dept = d })
            .SelectMany(empdept => grades
                .Where(s => empdept.Emp.Sal >= s.Losal && empdept.Emp.Sal <= s.Hisal)
                .Select(s => new { E = empdept.Emp, D = empdept.Dept, S = s }))
            .Select(eds => new { EName = eds.E.EName, DName = eds.D.DName, Grade = eds.S.Grade });
